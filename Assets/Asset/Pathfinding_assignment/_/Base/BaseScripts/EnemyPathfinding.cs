...
public class EnemyPathfinding : MonoBehaviour {
    ...
    public interface IEnemyTargetable {
        Vector3 GetPosition();
        void Damage(EnemyPathfinding attacker);
    }

    public static List<EnemyPathfinding> enemyList = new List<EnemyPathfinding>();

    public static EnemyPathfinding GetClosestEnemy(Vector3 position, float maxRange) {
        EnemyPathfinding closest = null;
        foreach (EnemyPathfinding enemy in enemyList) {
    ...
    public static EnemyPathfinding Create(Vector3 position) {
        Transform enemyTransform = Instantiate(GameAssets.i.pfEnemy, position, Quaternion.identity);

        EnemyPathfinding enemyHandler = enemyTransform.GetComponent<EnemyPathfinding>();
